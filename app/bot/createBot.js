...
const createBot = (game, { config, settings }, winSwitch, state) => {
    ...
    const missOnPurposeValue = config.missOnPurpose
        ? random(config.missOnPurposeRandom.from, config.missOnPurposeRandom.to)
        : 0;
    ...
    const notificationZone = createNotificationZone({
        ...
        zone: Zone.from({
            x: Math.round(
                screenSize.width / 2 -
                    screenSize.width * config.notificationPos.width
            ),
            y: Math.round(screenSize.height * config.notificationPos.y),
            width: Math.round(
                screenSize.width * config.notificationPos.width * 2
            ),
            height: Math.round(
                screenSize.height * config.notificationPos.height
            ),
        }),
    });
    ...
    let lootWinResType =
        screenSize.width <= 1536
            ? `1536`
            : screenSize.height == 1440 && config.lootWindow[`2560`]
            ? `2560`
            : `1920`;

    let ultrawide = screenSize.width / screenSize.height > 2;

    const lootWindowPatch = config.lootWindow[lootWinResType];
    ...
    const lootWindow = {
        ...
        toItemX:
            lootWindowPatch.toItemX *
            (ultrawide ? Number(lootWinResType) : screenSize.width),
        toItemY: lootWindowPatch.toItemY * screenSize.height,
        width:
            lootWindowPatch.width *
            (ultrawide ? Number(lootWinResType) : screenSize.width),
    ...
    if (lootWindowPatch.itemHeightAdd) {
        lootWindow.itemHeightAdd =
            lootWindowPatch.itemHeightAdd * screenSize.height;
    }
    ...
    const moveTo = async ({
        ...
        if (config.likeHuman) {
            ...
            let randomSpeed = {
                from: speedByRes - speedCoofByRes,
                to: speedByRes + speedCoofByRes,
            };
            ...
            let randomDeviation = {
                from: config.mouseCurvatureStrength - deviationCoof,
                to: config.mouseCurvatureStrength + deviationCoof,
            };
            ...
            if (config.likeHumanFineTune && fineTune) {
                ...
                for (let i = 1; i <= times; i++) {
                    await mouse.humanMoveTo(
                        pos.x +
                            random(-fineTune.offset / i, fineTune.offset / i),
                        pos.y +
                            random(-fineTune.offset / i, fineTune.offset / i),
    ...
    const logOutTimer = createTimer(() =>
        random(
            config.logOutEvery.from * 1000 * 60,
            config.logOutEvery.to * 1000 * 60
        )
    );
    ...
    const logOut = async (state) => {
        ...
        await sleep(
            random(config.logOutFor.from * 1000, config.logOutFor.to * 1000)
        );
    ...
    const preliminaryChecks = async () => {
        ...
        if (screenSize.x == -32000 && screenSize.y == -32000) {
            throw new Error(
                "The window is either in fullscreen mode or minimized. Switch to windowed or windowed(maximized)."
            );
        }
        if (!settings.autoTh) {
            ...
            if (bobber) {
                ...
                const highlightRegion = new Region(
                    screenSize.x + (bobber.x - 30),
                    screenSize.y + (bobber.y - 30),
                    30,
                    30
                );
                ...
                throw new Error(
                    `Found ${
                        settings.bobberColor == `red` ? `red` : `blue`
                    } colors before casting. Change your Fishing Zone or increase the Threshold value or change the fishing place.`
                );
            }
        }
    };
    ...
    const findAllBobberColors = async () => {
        if (
            settings.game != `Retail` &&
            settings.game != `LK Classic` &&
            settings.game != `Classic` &&
            settings.game != `Vanilla (splash)`
        ) {
    ...
    const castFishing = async (state) => {
        ...
        if (state.status == "initial") {
            await sleep(250);
            if (
                !config.ignorePreliminary &&
                (await notificationZone.check("error"))
            ) {
                throw new Error(
                    `Game error notification occured on casting fishing.`
                );
            } else {
    ...
    const destroyJunk = async () => {
        await action(async () => {
            await keyboard.sendKey(settings.destroyKey, delay);
        });
    ...
    const highlightBobber = async (pos) => {
        if (
            settings.useInt ||
            (config.likeHuman &&
                random(0, 100) > Number(config.highlightPercent))
        ) {
            return pos;
        }
        ...
        if (config.reaction) {
            await sleep(
                random(config.reactionDelay.from, config.reactionDelay.to)
            );
        }
    ...
    const isLootOpened = async (cursorPos) => {
        ...
        if (config.multipleWindows) {
            ...
            let color = await screen.colorAt(
                new Point(x + screenSize.x, y + screenSize.y)
            );
    ...
    const pickLoot = async () => {
        let cursorPos =
            settings.atMouse || !lootWindow.cursorPos
                ? mouse.getPos()
                : lootWindow.cursorPos;
        ...
        if (config.reaction) {
            await sleep(
                random(config.reactionDelay.from, config.reactionDelay.to)
            ); // hint disappearing and smooth text analyzing time with random value
        } else {
        ...
        let recognizedWords = await readTextFrom(
            await getDataFrom(lootWindowDim),
            screenSize.width <= 1536 ? 3 : 2
        );
        let items = sortWordsByItem(
            recognizedWords,
            lootWindow,
            settings.game == `Retail`
        );
        ...
        for (let item of items) {
            let isInList = whitelist.find(
                (word) => percentComparison(word, item) > 90
            );
            ...
            if (isInList) {
                ...
                if (config.reaction && random(0, 10) > 8) {
                    // sometimes we think
                    await sleep(
                        random(
                            config.reactionDelay.from,
                            config.reactionDelay.to
                        )
                    );
                } else {
                ...
                if (typeof isInList == `boolean` && settings.confirmSoulbound) {
                    ...
                    if (
                        recognizedWords.some(
                            (item) => percentComparison(`Okay`, item.text) > 75
                        )
                    ) {
                        await moveTo({
                            pos: {
                                x:
                                    confirmationWindow.x +
                                    confirmationWindow.width / 2,
                                y:
                                    confirmationWindow.y +
                                    confirmationWindow.height / 2,
                            },
                            randomRange: 5,
                        });
            ...
            itemPos +=
                settings.game == `Retail`
                    ? lootWindow.itemHeight + lootWindow.itemHeightAdd
                    : lootWindow.itemHeight;
        }
        ...
        if (
            ...
            settings.game == `Classic` ||
            settings.game == `Retail`
                ? await isLootOpened(cursorPos)
                : items.length != itemsPicked.length
        ) {
            if (config.reaction) {
                await sleep(
                    random(config.reactionDelay.from, config.reactionDelay.to)
            ...
            if (
                (config.closeLoot == `mouse` ||
                    config.closeLoot == `mouse+esc`) &&
                lootWindow.exitButton
            ) {
    ...
    const hookBobber = async (pos) => {
        ...
        if (!pos) {
            // in case recasting if it's stated in maxFishTimeAfter
            return caught;
        }
        ...
        if (config.sleepAfterHook) {
            await sleep(
                random(config.afterHookDelay.from, config.afterHookDelay.to)
            );
        }
