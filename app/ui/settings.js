    ...
    constructor(config) {
        ...
        this.dom.addEventListener("input", (event) => {
            if (event.target.name == `threshold`) {
                this.dom
                    .querySelectorAll("input[name=threshold]")
                    .forEach((node) => {
        ...
        this.dom.addEventListener("mousedown", (event) => {
            if (
                (event.target.name == `stopKey` ||
                    event.target.name == `fishingKey` ||
                    event.target.name == `destroyKey` ||
                    event.target.name == `luresKey` ||
                    event.target.name == `intKey`) &&
        ...
        this.dom.addEventListener("click", (event) => {
            if (
                event.target.name == `bobberColor` ||
                event.target.parentNode.name == `bobberColor`
            ) {
                let bobberColorNode =
                    this.dom.querySelector(`.bobberColorSwitch`);
                bobberColorNode.value =
                    bobberColorNode.value == `red` ? `blue` : `red`;
            ...
            if (
                event.target.name == `autoTh` ||
                event.target.parentNode.name == `autoTh`
            ) {
            ...
            if (
                event.target.name == `multipleWindows` &&
            ...
            if (
                event.target.name == `lures` &&
                event.target.checked &&
                (config.game == `Retail` ||
                    config.game == `Classic` ||
                    config.game == `Vanilla` ||
                    config.game == `Vanilla (splash)`)
            ) {
                this.onLures();
            }

            if (
                event.target.name == `whitelist` &&
                event.target.checked == true
            ) {
